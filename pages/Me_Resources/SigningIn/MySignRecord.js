...
Page({
  ...
  getlist: function () {
    ...
    comm.unitWebsitePro('PostAttenDance', tempData, function (data) {
      if (data.RspCode == "0000") { //正常  
        if (Flag == 0) {
          that.setData({
            cardrecordlist: data.RspData.signlist, //绑定数据列表
            isxs: true //显示加载更多
          });
        } else {
          that.setData({ //加载更多数组拼接
            cardrecordlist: that.data.cardrecordlist.concat(data.RspData.signlist),
            isxs: true
          });
        }
        pageindex++;
  ...
  //列表详情
  mingxi: function (e) {
    var arr = e.currentTarget.dataset.can;  
    var strarr=JSON.stringify(arr)
    wx.navigateTo({ //页面跳转
      url: '/pages/Me_Resources/SigningIn/Recorddetails?can=' + strarr,
    })
  },
