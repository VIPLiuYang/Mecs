...
Page({
  ...
  showTopTips: function (e) {
    ...
    for (var i = 0; i < arr.length; i++) {
      xmls += '<node orderItemId="' + arr[i]["<ProductId>k__BackingField"] + '" ProNo="' + arr[i]["<proNo>k__BackingField"] + '" ckNum="' + arr[i].count + '" dgPrice="' + arr[i].price + '" />'; 
    }
    xmls += '</root>';
    ...
    comm.unitWebsitePro('PostInventoryList', tempData, function (data) {
      ...
      if (bool == "0000") {
        ...
        wx.navigateTo({
          url: '/pages/Me_SalesGoods/Purchase/Purchase',
        })
      } else {
